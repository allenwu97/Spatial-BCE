...
class Net(nn.Module):
    ...
    def forward_as_dict(self, x):
        ...
        x, p3 = self.b4(x, get_x_bn_relu=True)
        x = self.b4_1(x)
        x = self.b4_2(x)
        x = self.b4_3(x)
        x = self.b4_4(x)
        ...
        x, p4 = self.b5(x, get_x_bn_relu=True)
        x = self.b5_1(x)
        x = self.b5_2(x)
...
class ResNet38(Net):
    def __init__(self):
        super().__init__()
        self.fc_list = nn.ModuleList([nn.Conv2d(128, 1, 1, bias=False) for _ in range(20)])
        for fc in self.fc_list:
            torch.nn.init.xavier_uniform_(fc.weight)
        self.temperature = nn.Parameter(torch.tensor([torch.log(torch.tensor(10.)) for _ in range(20)]))

        self.threshold = nn.Conv2d(4096, 20, 1, 1, bias=False)
        torch.nn.init.xavier_uniform_(self.threshold.weight)

        self.mask_layer_downsample = nn.Conv2d(4096, 128, 1, bias=False)
    ...
    def forward(self, x):
        ...
        for i in range(20):
            feature_ = self.fc_list[i](x)
            label_feature.append(feature_)
        x = torch.cat(label_feature, 1)
    ...
    def forward_cam(self, x):
        ...
        x = self.mask_layer_downsample(x)
        for i in range(20):
            cam = F.conv2d(x, self.fc_list[i].weight)
            cam_list.append(cam)
        x = torch.cat(cam_list, dim=1)
